#Jab hum PDF ya kisi document se text nikalte hain, wo text kaafi lamba (long) hota hai — jaise 5000 words ka ek block. Lekin LLM ya embedding models chhote-chhote pieces (chunks) me kaam karte hain behtar. Isliye humein us bade text ko chhote tukdon me todna (split karna) padta hai. Yahi kaam CharacterTextSplitter karta hai.
...
splitter = CharacterTextSplitter(
    chunk_size = 100,#Ye batata hai ki har chunk (tukda) me maximum 100 characters honge.
    chunk_overlap = 0,#Ye batata hai ki do consecutive (aage-peeche wale) chunks ke beech kitna text common (overlap) hoga.0 ka matlab: koi overlap nahi. Har chunk bilkul alag hoga, kuch bhi repeat nahi hoga.Agar aap chunk_overlap = 20 karte, to har chunk ke last 20 characters, agle chunk ke start me bhi repeat hote — isse context break nahi hota beech me
    separator = ''  #Ye batata hai ki text ko kis cheez ke basis pe todna hai.Normally separator="\n" hota hai (matlab naye line pe todo), ya separator=" " (space pe todo, taaki beech me se word na tute).Yahan separator='' (empty string) ka matlab hai: koi specific separator follow mat karo, bas seedha character count (100) ke hisaab se kaat do — chahe beech me word tut jaaye.
)
